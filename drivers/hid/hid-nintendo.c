...
/*
 * Backport to Linux 4.4 (the PlayStation Classic's kernel): the 6.1 driver with
 * local compatibility shims. The shared headers are left alone, so the device
 * ids live here and the LED differences are handled below.
 */
#ifndef USB_VENDOR_ID_NINTENDO
#define USB_VENDOR_ID_NINTENDO		0x057e
#endif
#ifndef USB_DEVICE_ID_NINTENDO_JOYCONL
#define USB_DEVICE_ID_NINTENDO_JOYCONL	0x2006
#endif
#ifndef USB_DEVICE_ID_NINTENDO_JOYCONR
#define USB_DEVICE_ID_NINTENDO_JOYCONR	0x2007
#endif
#ifndef USB_DEVICE_ID_NINTENDO_PROCON
#define USB_DEVICE_ID_NINTENDO_PROCON	0x2009
#endif
#ifndef USB_DEVICE_ID_NINTENDO_CHRGGRIP
#define USB_DEVICE_ID_NINTENDO_CHRGGRIP	0x200E
#endif

/* include/dt-bindings/leds/common.h's function names, 5.x */
...
/*
 * LED_HW_PLUGGABLE (4.5) only stops the core from switching the LED off when
 * it is unregistered; without it, 4.4 calls brightness_set on removal, which
 * the shim below ignores once the controller is marked removed.
 */
#ifndef LED_HW_PLUGGABLE
#define LED_HW_PLUGGABLE 0
#endif

/* to_hid_device() arrived in 4.5; 4.4 spells it container_of */
#ifndef to_hid_device
#define to_hid_device(pdev) container_of(pdev, struct hid_device, dev)
#endif

/*
 * 4.4's LED core has no brightness_set_blocking (4.5): brightness_set must not
 * sleep, and setting a Switch controller's LED is a synchronous subcommand.
 * JC_LEGACY_LEDS makes brightness_set record which LED changed and do the
 * send from a work item, as the 4.5+ core does for a blocking setter.
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(4, 5, 0)
#define JC_LEGACY_LEDS 1
#else
#define JC_LEGACY_LEDS 0
#endif
...
#if JC_LEGACY_LEDS
	/* bits 0..JC_NUM_LEDS-1: a player LED changed; JC_NUM_LEDS: home LED */
	unsigned long led_pending;
...
static void joycon_led_worker(struct work_struct *work)
{
	...
	/* one send covers every player LED: it writes all four bits */
	for (i = 0; i < JC_NUM_LEDS; i++) {
		if (test_and_clear_bit(i, &ctlr->led_pending)) {
			int j;

			for (j = i + 1; j < JC_NUM_LEDS; j++)
				clear_bit(j, &ctlr->led_pending);
			joycon_player_led_brightness_set(&ctlr->leds[i],
							 ctlr->leds[i].brightness);
			break;
		}
	}
	if (test_and_clear_bit(JC_NUM_LEDS, &ctlr->led_pending))
		joycon_home_led_brightness_set(&ctlr->home_led,
					       ctlr->home_led.brightness);
}

static void joycon_led_brightness_set(struct led_classdev *led,
				      enum led_brightness brightness)
{
	...
	/* the core has stored the new value in led->brightness already */
	for (i = 0; i < JC_NUM_LEDS; i++) {
		if (&ctlr->leds[i] == led)
			bit = i;
	}
	...
	spin_lock_irqsave(&ctlr->lock, flags);
	if (ctlr->ctlr_state != JOYCON_CTLR_STATE_REMOVED) {
		set_bit(bit, &ctlr->led_pending);
		schedule_work(&ctlr->led_worker);
	}
	spin_unlock_irqrestore(&ctlr->lock, flags);
}

#define joycon_set_led_op(led, fn) \
	((led)->brightness_set = joycon_led_brightness_set)
#else
#define joycon_set_led_op(led, fn) ((led)->brightness_set_blocking = (fn))
#endif

/*
 * Mark the controller removed (no more LED work gets queued, subcommands
 * return -ENODEV) and wait for queued LED work, before devres unregisters the
 * LEDs and frees ctlr.
 */
static void joycon_mark_removed(struct joycon_ctlr *ctlr)
{
	unsigned long flags;
	...
	spin_unlock_irqrestore(&ctlr->lock, flags);
#if JC_LEGACY_LEDS
	cancel_work_sync(&ctlr->led_worker);
#endif
}
...
static int joycon_leds_create(struct joycon_ctlr *ctlr)
{
	...
	for (i = 0; i < JC_NUM_LEDS; i++) {
		...
		joycon_set_led_op(led, joycon_player_led_brightness_set);
	...
	if (jc_type_has_right(ctlr)) {
		...
		joycon_set_led_op(led, joycon_home_led_brightness_set);
		led->flags = LED_CORE_SUSPENDRESUME | LED_HW_PLUGGABLE;
		ret = devm_led_classdev_register(&hdev->dev, led);
		if (ret) {
